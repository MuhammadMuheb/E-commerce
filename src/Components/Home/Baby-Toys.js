...
export default function BabysToys() {
  const { addItem } = useCart();
  const navigate = useNavigate();
  const [babystoys, setBabysToys] = useState([]);
  // const babystoys = []
  useEffect(()=>{
          fetch("https://e-commerce-app-33918-default-rtdb.firebaseio.com/babystoys.json")
          .then((response) =>{
